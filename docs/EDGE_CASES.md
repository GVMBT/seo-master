...
| E08 | VK OAuth-токен невалиден (отозван / refresh failed) | При ошибке API → попробовать refresh_token. Если refresh failed → пометить connection status=error, уведомить "[Переподключить VK]" (повторный OAuth) |
