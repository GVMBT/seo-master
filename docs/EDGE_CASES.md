...
| E38 | Генерация N отзывов, баланс < N × 10 | "Недостаточно токенов. Нужно {N×10}, у вас {balance}. [Пополнить] или [Меньше отзывов]". Проверка в ReviewGenerationFSM.confirm_cost |
...
| # | Сценарий | Ожидаемое поведение |
|---|---------|---------------------|
| E49 | Pipeline прерван на mid-step (FSM timeout 30 мин) | Redis checkpoint (`pipeline:{user_id}:state`, TTL 24ч) жив — при возврате пользователя предложить "Продолжить" / "Начать заново" / "Отменить" (§16.10). После 24ч — checkpoint удалён, начать заново |
