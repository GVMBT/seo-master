...
| E08 | VK token невалиден — community или personal (отозван / истёк) | При ошибке VK API → пометить connection status=error, уведомить "[Переподключить VK]" (ввести новый community API token или пройти OAuth для personal page). Refresh_token не используется (manual token flow) |
