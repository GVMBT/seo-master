...
|--------|----------|-----------|---------------------|
| Nano Banana 2 (Gemini 3.1 Flash Image) | `google/gemini-3.1-flash-image-preview` | Основная генерация (Pro-качество, Flash-скорость) | $0.25/$1.50 per M tokens, $60/M image output |
| Nano Banana (Gemini 2.5 Flash Image) | `google/gemini-2.5-flash-image` | Fallback | $0.30/$2.50 per M tokens, $30/M image output |

**Fallback цепочка:** определяется через `MODEL_CHAINS["image"]` (§3.1): Nano Banana 2 → Nano Banana → ошибка + возврат токенов.
