...
### 4.1. Полная схема (ArticlePipelineFSM — 23 состояния)
...
[Написать статью на сайт]
  ...
  │   ├── >1 подключений → "Статья > WordPress
  │   │    На какой сайт?"
  │   │    [comfort-mebel.ru]  [blog.comfort.ru]
  │   └── 0 подключений → "Статья > WordPress
...
### 5.1. Полная схема (SocialPipelineFSM — 27 состояний)
...
Шаг 8 (после публикации):
  "✅ Статья опубликована!
   ...
   [Каждый будний день в 10:00]     ← SUCCESS — quick preset
...
"⚠️ Недостаточно токенов!
 ...
 [Пополнить баланс] ← SUCCESS  → экран Токены → после оплаты → автовозврат
 [Отмена]"
```
...
```
"⚠️ Бот не является администратором @comfort_channel.
...
```
"⚠️ Ошибка VK API: {описание ошибки}
...
```
"⚠️ Pinterest ограничил запросы. Попробуйте через 15 минут.
...
```
"⚠️ Не удалось опубликовать в {платформа}: {канал}.
...
routers/publishing/pipeline/
  ...
  social.py             — SocialPipelineFSM (27 состояний)
...
Каждое сообщение pipeline начинается с компактного заголовка:
...
|--------|-------|
| Кнопки подключений | — |
| `[Подключить Telegram/VK/Pinterest]` | `SUCCESS` |
...
| Экран | Кнопка | Style |
|-------|--------|-------|
| Нехватка токенов | `[Пополнить баланс]` | `SUCCESS` |
...
| Кнопка | Style |
|--------|-------|
| `[Каждый будний день]` / `[Пн, Ср, Пт]` | `SUCCESS` |
