...
[Написать статью на сайт]
  ...
  │   │
  │   ├── 4a. Ключевые фразы (бесплатно, без подтверждения)
  │   │    [Подобрать автоматически]   → auto-fill из category + project
  │   │    [Настроить параметры]       → products → geography → генерация
  │   │    [Загрузить свои]            → KeywordUpload
  │   │    [Назад к чеклисту]
  │   │    │
  │   │    ├── [Подобрать автоматически] → category.name как products.
  │   │    │    Если нет company_city — один доп. вопрос:
  │   │    │    "В каком городе ваш бизнес? (для точных SEO-фраз)"
  │   │    │    [Москва] [СПб] [Ввести свой] [Вся Россия]
  │   │    │    Город сохраняется в project.company_city.
  │   │    │    Если city есть — генерация запускается сразу (без qty/confirm).
  │   │    │    → автовозврат к чеклисту
  │   │    ├── [Настроить] → products → geo → генерация → автовозврат к чеклисту
...
[Опубликовать пост в соцсети]
  ...
  └─ ШАГ 7: Результат + next actions
      ...
      [Открыть пост ↗]                         ← URL-кнопка (если есть ссылка)
      [Кросс-пост (~10 ток.)]                  ← если есть другие social-подключения
      [Главное меню]

      Кнопка кросс-поста (одна) показывается, если у проекта есть
      хотя бы ещё одно social-подключение (кроме текущего и WordPress).
      При нажатии — экран выбора платформ (§6), затем AI-адаптация + ревью.
...
После публикации поста предлагается адаптация для других подключённых платформ.
Одна кнопка `[Кросс-пост (~10 ток.)]` ведёт на экран выбора платформ.
...
[Кросс-пост (~10 ток.)]
  → "На какие платформы адаптировать?
     ...
     [✓ VK]  [✓ Pinterest]        ← toggle, все включены по умолчанию
     [Адаптировать] ← SUCCESS     [Отмена]"
```

callback_data: `pipeline:crosspost:toggle:{conn_id}`, `pipeline:crosspost:go`, `pipeline:crosspost:cancel`

Если доступно ровно одно целевое social-подключение (кроме текущего источника) — экран выбора пропускается, сразу адаптация.

### 6.2. Стоимость

- Кросс-пост: `ceil(adapted_word_count / 100) * 10` (~10 токенов за платформу)
...
### 6.3. Обязательный ревью

Адаптированные посты проходят ревью **по очереди** перед публикацией:
...
"Пост адаптирован для VK:
 ...
 [Опубликовать в VK] ← SUCCESS    [Пропустить]    [Отмена]"
```

callback_data: `pipeline:crosspost:{conn_id}:publish`, `pipeline:crosspost:{conn_id}:skip`, `pipeline:crosspost:cancel`

После публикации/пропуска — следующая платформа, или результат.
...
"Ошибка VK API: {описание ошибки}
 Возможно, OAuth-токен истёк или группа заблокирована.
...
/start (Dashboard)
 ...
 │   ├──[Подтверждение]
 │   ├──[Ревью] (текст + фото при наличии; Pinterest: вертикальное 2:3) → [Опубликовать] / [Перегенерировать] / [Отмена]
 │   └──[Результат] → [Кросс-пост (~10 ток.)] / [Ещё пост] / [Меню]
