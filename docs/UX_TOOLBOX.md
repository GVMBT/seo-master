...
### 6.3. VK (OAuth 2.1 PKCE)

```
ConnectVKFSM:
  Шаг 1: [Авторизоваться в VK] → открывает браузер (id.vk.com OAuth 2.1 + PKCE S256)
           ┌─────────────────────────────┐
           │ VK ID                       │
           │                             │
  ...
  Шаг 2: Автоматический возврат в бота (deep-link /start vk_auth_{nonce})

  Шаг 3: Список групп (admin/editor):
    [Мебель Комфорт (1500 участников)]
    [Автосервис Гараж (300 участников)]
    → Одно нажатие = 1 запись в platform_connections

  → Успех: "VK подключён! Группа: Мебель Комфорт"
  → Если нет групп: "У вас нет групп VK, в которых вы администратор."
  → При ошибке авторизации: "Не удалось подключить VK. Попробуйте снова."
  → TTL авторизации: 30 мин (Redis, E20)
```
...
```
KeywordGenerationFSM (auto-fill, no quantity selection):
  Товары/услуги: auto-fill из category.description или category.name
  География: auto-fill из project.company_city (если заполнен)

  Если company_city не заполнен:
    "Укажите географию продвижения (2-200 символов)"
    География сохраняется в project.company_city для будущих запусков.

  → Генерация запускается сразу (без подтверждения, без выбора количества)

  → Прогресс:
    "Получаю реальные фразы из DataForSEO..."
    "Группирую по интенту (до 1.5 мин)..."
    "Обогащаю данными..."

  → Результат: сводка по кластерам (volume=0 скрыт)
```
...
"Токены
 ...
 Справочник стоимости:
   ...
   Подбор ключевых фраз: бесплатно"
...
"Админ-панель
 ...
 Проектов: 89
 Публикаций (7д): 42
 Затраты API (30д): $156.30"

[Мониторинг]            → БД, Redis, активные расписания QStash
[Просмотр пользователя] → ID или @username → карточка пользователя
[Рассылка]              → выбор аудитории → текст → подтверждение
[Затраты API]           → расходы за 7/30/90 дней
[Главное меню]
```

Мониторинг показывает статус БД (запрос), Redis (ping), количество активных расписаний.
Логи ошибок — через Sentry (внешний сервис), не дублируются в боте.

callback_data: `admin:monitoring`, `admin:user_lookup`, `admin:broadcast`, `admin:api_costs`, `nav:dashboard`
...
[Мои проекты] (Dashboard)
 ...
 │         │    ├── [Добавить VK] → ConnectVKFSM (OAuth 2.1 PKCE)
