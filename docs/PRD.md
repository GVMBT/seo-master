...
> - [EDGE_CASES.md](EDGE_CASES.md) — edge cases (E01-E25), обработка ошибок, валидация контента
...
| F09 | AI-генерация ключевых фраз + реальные SEO-данные | Семантическое ядро: 50-200 фраз с реальным объемом поиска и сложностью (DataForSEO) | P0 |
...
| F31 | Ключевые фразы без реальных данных | AI генерирует "из головы", нет объема поиска/сложности | DataForSEO: реальный объем поиска + сложность + похожие фразы | DataForSEO |
...
| F36 | Версионирование промптов | YAML-промпты с A/B тестированием | P1 |
...
**Решение через Firecrawl:** Эндпоинт /extract со схемой.

```python
# Извлечение SEO-структуры конкурента
...
1. **SEO-аудит конкурентов** — анализ мета-тегов, заголовков, структуры контента конкурентов
2. **Анализ контентных пробелов** — какие ключевые слова покрывает конкурент, а мы нет?
3. **Извлечение цен** — парсинг цен конкурентов со страниц товаров
4. **Сбор отзывов** — сбор реальных отзывов с Google/Яндекс для обучающих данных
...
**Текущая проблема:** AI генерирует ключевые фразы "из головы". Пользователь получает список из 100 слов, но не знает — ищет ли их вообще кто-нибудь в Яндексе/Google.

**Решение через DataForSEO:** Реальные данные из поисковых систем.

**Что получаем:**
- Объем поиска (сколько раз в месяц ищут эту фразу)
- Сложность ключевика (насколько трудно попасть в топ-10)
- Похожие фразы (расширение семантического ядра реальными запросами)
- CPC (сколько стоит клик в рекламе — показатель коммерческой ценности)

**Новый пайплайн генерации ключевых фраз:**
```
1. AI генерирует 100 фраз (как сейчас)
2. DataForSEO обогащает каждую фразу реальными данными
3. Пользователь видит таблицу:

   Фраза                           | Запросов/мес | Сложность | Ценность
   кухни на заказ москва            | 12 400       | Высокая   | $$$
   кухонный гарнитур из массива     | 3 200        | Средняя   | $$
   модульная кухня купить недорого  | 890          | Низкая    | $

4. Бот автоматически сортирует: сначала фразы с высоким объемом и низкой сложностью
5. Для автопубликации: бот сначала берет самые перспективные фразы
```

**Стоимость:** $0.0001 за ключевое слово. 100 фраз = $0.01 (~1 рубль). Включаем бесплатно для пользователя.
...
**Новый пайплайн генерации статей:**
```
1. Бот берет ключевое слово из категории
2. Serper делает 2-3 запроса в Google:
   - Поиск по ключевому слову → топ-10 результатов
   - "Люди также спрашивают" → готовые вопросы для FAQ
   - Новости по теме → актуальные данные для статьи
3. Результаты Serper + данные компании + прайс + отзывы → промпт для AI
4. AI пишет статью на основе реальных актуальных данных
```
...
- Статья лучше ранжируется, потому что отвечает на реальные запросы

**Стоимость:** Бесплатно до 2 500 запросов/мес. При 3 запросах на статью — хватает на ~830 статей/мес.
...
| Ключевые фразы | AI-фантазии без данных | DataForSEO (реальные объемы/сложность) | Фразы с реальной ценностью |
| Web Search для статей | Предусмотрено, но сломано | Serper (данные Google в реальном времени) | Актуальные статьи |
...
Пользователь нажимает "Опубликовать статью"
  ...
  │     │     ~320 токенов (200 за текст + 120 за 4 изображения)
...
- Начисление мгновенно после successful_payment
...
| Генерация ключевых фраз (50-200 шт) | 50-200 | Сохраняем |
...
| Данные пользователей | Supabase RLS (безопасность на уровне строк) |
...
| Дашборд Supabase | Метрики БД, политики RLS |
...
| Регрессия качества промптов | 15% | Среднее | A/B тестирование, сохранение промпта v4 как резервного |
...
| OpenRouter (AI-модели) | По потреблению | ~$200-500 |
| Sentry | Team | $26 |
| Домен + SSL | - | $15 |
| **Итого** | | **$404-724/мес** |
...
| Валовая маржа | ($3 000 - $724) / $3 000 | **~76%** |
