...
> - [EDGE_CASES.md](EDGE_CASES.md) — edge cases (E01-E52), обработка ошибок, валидация контента
> - [UX_PIPELINE.md](UX_PIPELINE.md) — Pipeline UX: Dashboard, статьи, соцсети, кросс-постинг
> - [UX_TOOLBOX.md](UX_TOOLBOX.md) — Toolbox UX: проекты, категории, подключения, профиль, токены
>
> **История версий:**
...
| F05 | Пакеты токенов и биллинг | 3 пакета (Старт 500, Стандарт 2000, Про 5000), 1 токен = 1 руб., скидки за объём | P0 |
...
| F13 | Публикация в VK | OAuth 2.1 PKCE (id.vk.com). wall.post, загрузка фото, поддержка групп. refresh_token для автообновления | P0 |
...
| ~~F17~~ | ~~Генерация отзывов~~ | ~~Отложено до v3~~ | — |
...
| F23 | AI-генерация изображений | Генерация AI-изображений через OpenRouter (Nano Banana). Загрузка пользовательских медиа удалена | P1 |
...
| F27 | Краулинг сайта (внутренние ссылки) | Работает, но СИНХРОННО (блокирует 60с, макс 50 страниц) | Firecrawl `/map` — до 5000 URL за 2-3с, 1 кредит | Firecrawl |
...
| F32 | Web Search в статьях | Реализовано | Serper API (PAA + organic) + Firecrawl /scrape (конкуренты) → websearch-фаза в pipeline | Serper + Firecrawl |
...
| F34 | ~~AI-стриминг~~ | ~~Генерация статьи в реальном времени видна пользователю~~ (replaced by progress messages via editMessageText, deferred to v3 via sendMessageDraft) | P3 |
...
| F39 | ~~Анализ конкурентов~~ | ~~Firecrawl /extract для анализа сайтов конкурентов~~ Deferred to v3. Контентный gap-анализ конкурентов выполняется автоматически в пайплайне статей (Firecrawl /scrape) | ~~P2~~ |
...
| F42 | ~~Быстрая публикация в 1 клик~~ | Replaced by Goal-Oriented Pipeline (см. [UX_PIPELINE.md](UX_PIPELINE.md)). Pipeline: "Написать статью" / "Пост в соцсети" → readiness check → генерация → публикация | P1 |
...
| F45 | ~~Мониторинг изменений контента~~ | Отложено до v3. Firecrawl `changeTracking` на `/scrape` для мониторинга конкурентов. В v2 спецификация отсутствует | P3 |
| ~~F46~~ | ~~Помощь (встроенная справка)~~ | ~~Удалено — onboarding через Pipeline UX~~ | — |
...
#### 5.1.2 Внутренние ссылки сайта (заменяет utils/site_crawler.py)
...
**Решение через Firecrawl `/map`:** Быстрое обнаружение URL без скрейпинга контента.

```python
# 1 кредит за до 5000 URL, ответ за 2-3 секунды
result = await firecrawl.map_site(
    url='https://сайт-клиента.ru',
    limit=5000,
)
...
**Почему `/map` вместо `/crawl`:**
Для internal links нужны только URL + title, не контент каждой страницы.
- `/map`: **1 кредит** за до 5000 URL, 2-3 секунды
- `/crawl`: 1 кредит **за страницу**, 100 стр = 100 кредитов, 30+ секунд

**Улучшения по сравнению с текущим:**
- В 100 раз дешевле `/crawl` (1 vs 100 кредитов за сайт)
- 2-3 секунды вместо 60 секунд
- До 5000 URL вместо 50
- Не блокирует обработчик
- JS-рендеринг (обрабатывает React/Vue/Next.js сайты)

**Стоимость:** 1 кредит за вызов = ~$0.001 на тарифе Standard. Было: $0.08/сайт.

> **Примечание:** `/crawl` остаётся в арсенале для P2-сценариев, где нужен полный контент всех страниц (например, полный аудит контента сайта).
...
**Решение через Firecrawl:** Пайплайн статей v7 использует `/scrape` — полный markdown контента конкурентов для gap-анализа и динамической длины. Standalone-анализ конкурентов (F39) отложен до v3.

```python
# Article v7: Полный контент конкурента (для gap-анализа)
result = firecrawl.scrape(url='https://конкурент.ru/статья', formats=['markdown'])
```

**Сценарий использования:**
- **Контентный gap-анализ** (article v7, `/scrape`) — полный markdown для сравнения покрытия тем (автоматически при генерации статьи)

#### 5.1.4 Прогноз расходов на Firecrawl

| Операция | Частота | Кредитов/операция | Кредитов/мес | Стоимость/мес (тариф Standard $83) |
|----------|---------|-------------------|--------------|-------------------------------------|
| Извлечение брендинга (v2) | 50 новых сайтов/мес | 1 | 50 | < $0.05 |
| Внутренние ссылки (`/map`) | 50 сайтов/мес | 1 | 50 | < $0.05 |
| Перекраулинг ссылок (P2, 2×/мес) | 200 сайтов × 2 | 1 | 400 | ~$0.33 |
| Скрейп конкурентов (статьи) | 500 статей × 3 URL | 1 | 1 500 | ~$1.25 |
| **Итого** | | | **2 000** | **~$1.68** |

> **Экономия vs предыдущий план:** 14 050 → 2 000 кредитов/мес (−86%) благодаря замене `/crawl` на `/map` и отложению F39 в v3.

Тариф Standard (100К кредитов/$83) покрывает потребности с запасом 94К. Тариф Hobby (3К/$16) покрывает MVP.
...
**Модель монетизации: разовые пакеты токенов.**
Простая покупка пакетов со скидкой за объём. Без подписок — пользователь покупает когда нужно.
...
- **ЮKassa — альтернатива:** комиссия 3.5%, но требует редирект на внешнюю страницу оплаты. Предлагаем для пользователей, предпочитающих оплату картой.
- **UI:** на экране `[Токены]` пользователь выбирает пакет, затем способ оплаты: `[Оплатить Stars]` или `[Оплатить картой]`.

> Технические детали платёжного flow → см. [API_CONTRACTS.md](API_CONTRACTS.md) §2

**Пакеты токенов:**

| Пакет | Токены | Цена (руб.) | Скидка | Stars (приблизительно) |
|-------|--------|-------------|--------|----------------------|
| Старт | 500 | 500 | — | ~33 Stars |
| Стандарт | 2 000 | 1 600 | −20% | ~104 Stars |
| Про | 5 000 | 3 000 | −40% | ~195 Stars |

**Формула расчёта Stars:** Stars ≈ price_rub / 15.38 (курс ~15.38 руб/Star на стороне пользователя). **Источник истины — hardcoded таблица выше.** Актуальный курс зависит от курса USD и политики Telegram; таблицу обновлять вручную.
...
**Стоимость:** Serper ~$0.001/запрос (Starter $50/50K кредитов). Firecrawl /scrape: 3 кредита/статью ($0.003). При цене статьи 200 токенов (200 руб) — пренебрежимо.
...
| Краулер сайта | 171 строка, синхронный, базовый | Firecrawl `/map` (1 кредит, 5000 URL, 2-3с) | -171 строка, в 100 раз дешевле |
...
| Web Search для статей | Реализовано | Serper + Firecrawl /scrape (конкуренты) | Актуальные статьи, динамическая длина, контентные gaps |
...
> Все экраны и навигация → см. [UX_PIPELINE.md](UX_PIPELINE.md) и [UX_TOOLBOX.md](UX_TOOLBOX.md)
...
Пользователь подключает WordPress-сайт
  ...
  │     ├── Firecrawl Branding v2 → цвета, шрифты, логотип → site_brandings
  │     ├── Firecrawl /map → внутренние ссылки (до 5000 URL, 2-3с) → connections
...
Пользователь нажимает "Опубликовать статью"
  ...
  │     ├── Внутренние ссылки из Firecrawl /map
  │     ├── Цвета сайта из Firecrawl Branding v2 → CSS inline-стили
...
```
Пользователь -> [Токены] -> Выбор пакета (Старт / Стандарт / Про)
  │
  ├── [Оплатить Stars]
...
### 8.3 Настройки проекта vs профиль пользователя

```
"Карточка проекта" (4 кнопки + удаление):
  ├── [Редактировать] → все поля проекта (вкл. часовой пояс)
  ├── [Категории] → список категорий
  ├── [Подключения] → добавить/удалить WordPress/TG/VK/Pinterest
  ├── [Планировщик] → расписание автопубликации
  └── [Удалить проект] → подтверждение (DANGER)

"Профиль" (в главном меню — объединяет бывшие [Настройки] + профиль):
  ├── Баланс токенов
  ├── Последний пакет
  ├── Всего публикаций
  ├── [Уведомления: ВКЛ/ВЫКЛ] → переключатель
  └── Реферальная ссылка
```
...
### 8.5 VK: OAuth 2.1 PKCE

VK перешёл на OAuth 2.1 (id.vk.com), Implicit Flow больше не работает. Реализован полный OAuth 2.1 + PKCE (S256): бот отправляет кнопку-ссылку → пользователь авторизуется на id.vk.com → callback → обмен code на access_token + refresh_token → выбор группы. Аналогично Pinterest OAuth (§6.4 UX_TOOLBOX.md). access_token TTL 60 мин, refresh_token 180 дней.
...
- При оплате Stars: берётся рублёвая цена пакета из таблицы пакетов
  Пример: друг купил Стандарт (1600 руб / ~104 Stars) → реферер получает 160 бонусных токенов
- При оплате ЮKassa: берётся фактическая сумма платежа в рублях
  Пример: друг купил Стандарт за 1600 руб → реферер получает 160 бонусных токенов
- Начисление мгновенно после КАЖДОГО successful_payment
...
| Генерация ключевых фраз (50-200 шт) | 0 (бесплатно) | Расход платформы (~$0.025 DataForSEO + AI). Ключи — инфраструктура, не продукт |
| Технический аудит | 50 | Сохраняем |
| ~~Генерация отзывов (за 1 отзыв)~~ | ~~10~~ | ~~Отложено до v3~~ |
| Генерация описания категории | 0 (бесплатно) | Расход платформы (~$0.01 AI). Описание — подготовительный шаг |
...
| Внутренние ссылки (Firecrawl /map) | < 3с для 5000 URL |
...
### 10.5 Качество AI-контента

| Аспект | Подход |
|--------|--------|
| Multi-step генерация | Outline (DeepSeek) → Expand (Claude) → Conditional Critique (если score < 80) |
| Markdown → HTML | AI генерирует Markdown; `mistune` + `SEORenderer` → детерминистичный HTML |
| Quality scoring | Программный `ContentQualityScorer` (0-100): SEO, readability, structure, naturalness, depth |
| Anti-hallucination | `<VERIFIED_DATA>` блок в промпте + regex fact-checking (цены, контакты, статистика) |
| Anti-slop | Blacklist ~20 слов-паразитов AI в system prompt |
| Нишевая специализация | 15+1 ниш: YMYL-дисклеймеры (medical, legal, finance), тональность, стиль изображений |
| Image SEO | images_meta (alt, filename, figcaption) в AI JSON; WebP-конвертация; WP REST alt_text |
| Image post-processing | Pillow: UnsharpMask + Contrast + Color boost перед WebP |
| Content uniqueness | SimHash (64-bit) в `publication_logs.content_hash`; warning при Hamming ≤ 3 |
| Quality gates | Score ≥ 80 auto-OK; 60-79 critique; 40-59 warning; < 40 block + refund |

> Техническая реализация: article_v7.yaml, ContentQualityScorer, SEORenderer → см. [API_CONTRACTS.md](API_CONTRACTS.md) §3.7, §5
...
| Изменения API Firecrawl | 10% | Среднее | Абстрагировано за `services/external/firecrawl.py`, используем стабильные v2 эндпоинты (/scrape, /map, /extract) |
...
| Истечение VK-токена (OAuth) | 5% | Низкое | Автообновление через refresh_token (180 дней). При ошибке refresh → уведомление "[Переподключить VK]" |
...
| Firecrawl | Hobby (3К кредитов) или Standard (100К) | $16-83 |
| DataForSEO | По потреблению (~50К запросов) | ~$5-15 |
| Serper | Starter (50K кредитов, разово) | $50 (одноразово, ~4 года) |
| OpenRouter (AI-модели) | По потреблению | ~$1 000-1 500 (с учётом изображений ~63% бюджета) |
| Sentry | Team | $26 |
| Домен + SSL | - | $15 |
| **Итого** | | **$1 137-1 707/мес** |
...
| Средний чек | Пакет Стандарт в среднем | 1 600 руб. (~$16) |
| Месячная выручка | 100 * $16 | **$1 600/мес** |
| Валовая маржа | Зависит от масштаба | AI-стоимость ~$0.30/статья, маржа ~91% на генерации |
...
- [Firecrawl /scrape](https://docs.firecrawl.dev/features/scrape) — Скрейпинг (markdown, summary, branding v2)
- [Firecrawl /map](https://docs.firecrawl.dev/features/map) — Быстрое обнаружение URL (1 кредит / 5000 URL)
- [Firecrawl /extract](https://docs.firecrawl.dev/features/extract) — Извлечение структурированных данных
- [Firecrawl /search](https://docs.firecrawl.dev/features/search) — Веб-поиск + скрейпинг (потенциальная замена Serper, v3)
- [Firecrawl /agent](https://www.firecrawl.dev/agent) — AI-агент для автономного сбора данных (Research Preview)
- [Firecrawl Branding v2](https://www.firecrawl.dev/blog/branding-format-v2) — Извлечение брендинга (улучшенное лого)
