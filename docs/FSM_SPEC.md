...
class ProjectCreateFSM(StatesGroup):
    name = State()           # Единственный шаг: название проекта (company_name = name по умолчанию)

# routers/categories/keywords.py
class KeywordGenerationFSM(StatesGroup):
    products = State()       # Upload mode: text/file input
    geography = State()      # Only if project.company_city is not set
    # === Data-first pipeline states (UI progress stages, NOT separate API task_types) ===
    # products = category.description or category.name (auto-filled, no user question)
    # geography = project.company_city (auto-filled if set)
    # Keywords are free — no token charge, no quantity selection, no confirmation.
...
# routers/publishing/pipeline/article.py (Goal-Oriented Pipeline: статьи, 20 состояний)
...
|-----|-----|----------------|-----------|------------|
| ProjectCreateFSM | name | Текст | 2-100 символов | "Введите название от 2 до 100 символов" |
...
| KeywordGenerationFSM | geography | Текст | 2-200 символов (only if project.company_city not set) | "Введите от 2 до 200 символов" |
...
### KeywordGenerationFSM (data-first pipeline, free)

```
[Начать подбор]
    │
    ├──[company_city заполнен]──► fetching ──► "Получаю реальные фразы..."
    │                                │
    └──[company_city не заполнен]    │
         │                          │
         ▼                     [DataForSEO OK]     [E03: DataForSEO недоступен]
    geography                       │                          │
    "В каком городе?"               ▼                          ▼
    ...
    [сохраняет city                 │
     в project]                [AI OK, ~10с]
         │                          ▼
         ▼                     enriching ──► "Обогащаю данными..."
    fetching → ...                  │
                               [DataForSEO OK, ~2с]
                                    ▼
                               results ──► "N кластеров (M фраз)"
                                    │
                                    ▼
...
### ArticlePipelineFSM (Goal-Oriented Pipeline: статьи, 20 состояний)
...
select_project ──[выбрал проект]──► select_wp ──[выбрал WP]──► select_category ──[выбрал]──► readiness_check
  ...
  │                                    │                          │                              │
  ▼                                    ▼                          ▼                              [Нет ключевиков]
create_project_name ──► select_wp   connect_wp_url             create_category_name                │
                                       │                          │                              ▼
                                       ▼                          ▼                          readiness_keywords_products
                                   connect_wp_login            readiness_check                   │
                                       │                                                        ▼
  ...
  ├── readiness_keywords_products → _geo → _generating → readiness_check (обяз.)
...
### SocialPipelineFSM (Goal-Oriented Pipeline: соц. посты, 24 состояния)
...
create_project_name          [Подключить TG] → connect_tg_channel create_category_name        confirm_cost ──[Да]──► generating ──[OK]──► review
  ...
  ├── readiness_keywords_products → _geo → _generating → readiness_check
