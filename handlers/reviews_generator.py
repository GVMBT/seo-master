...
НАЧНИ ГЕНЕРАЦИЮ {count} ЕСТЕСТВЕННЫХ ОТЗЫВОВ:"""
    ...
    try:
        # Вызываем Claude API через новый импорт
        import anthropic
        import os
        
        api_key = os.getenv('ANTHROPIC_API_KEY')
        if not api_key:
            raise Exception("ANTHROPIC_API_KEY не установлен в .env")
...
def handle_save_reviews(call):
    ...
    try:
        ...
        if not db._check_connection_alive():
            print("⚠️ save_reviews: соединение потеряно, переподключаемся...")
            db._reconnect()
        
        # Получаем существующие отзывы
        db.cursor.execute("""
            SELECT reviews FROM categories WHERE id = %s
        """, (category_id,))
        result = db.cursor.fetchone()
        
        existing_reviews = []
        if result and result.get('reviews'):
            existing_reviews = result['reviews'] if isinstance(result['reviews'], list) else []
        ...
        db.cursor.execute("""
            ...
            SET reviews = %s::jsonb
            WHERE id = %s
        """, (json.dumps(all_reviews, ensure_ascii=False), category_id))
        db.conn.commit()
        
        # Проверяем что UPDATE затронул строки
        if db.cursor.rowcount == 0:
            print(f"⚠️ save_reviews: UPDATE не затронул строк! category_id={category_id}")
            # Пробуем найти категорию
            db.cursor.execute("SELECT id FROM categories WHERE id = %s", (category_id,))
            cat_check = db.cursor.fetchone()
            if not cat_check:
                print(f"❌ Категория {category_id} не найдена в БД!")
                safe_answer_callback(bot, call.id, "❌ Категория не найдена в БД", show_alert=True)
                return
        ...
        text = (
            ...
            "━━━━━━━━━━━━━━\n\n"
            f"📊 Добавлено: <b>{len(reviews_data)}</b> новых отзывов\n"
            f"📊 Всего отзывов: <b>{len(all_reviews)}</b>\n\n"
        ...
        print(f"❌ Ошибка сохранения отзывов: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
