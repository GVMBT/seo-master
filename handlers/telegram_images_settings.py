...
def handle_telegram_images_menu(call):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "🔙 Назад",
            callback_data=f"platform_images_menu_telegram_{category_id}_{bot_id}_{platform_id}"
        )
    )
...
def handle_back_to_telegram(call):
    ...
    # Редирект на меню настроек изображений Telegram
    call.data = f"platform_images_menu_telegram_{category_id}_{bot_id}_{platform_id}"
