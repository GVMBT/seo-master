...
def send_main_menu(chat_id, message_text=None):
    ...
    logger.debug(f" Проверка админа: chat_id={chat_id}, ADMIN_ID={ADMIN_ID}, равны={str(chat_id) == str(ADMIN_ID)}")
    if str(chat_id) == str(ADMIN_ID):
        logger.debug(f" ✅ Добавляем кнопку АДМИНКА для пользователя {chat_id}")
        btn_admin = types.KeyboardButton("🔐 АДМИНКА")
        markup.row(btn_admin)
    else:
        logger.debug(f" ❌ Кнопка АДМИНКА НЕ добавлена: {chat_id} != {ADMIN_ID}")
...
def handle_start(message):
    ...
    try:
        ...
        if not existing_user:
            ...
            # ОТПРАВЛЯЕМ ПРИВЕТСТВЕННОЕ СООБЩЕНИЕ О ТОКЕНАХ (только для новых!)
            ...
            welcome_gift_message = (
                ...
                f"━━━━━━━━━━━━━━\n"
                f"Токены уже на вашем счету! 🎉"
            )
            bot.send_message(message.chat.id, welcome_gift_message, parse_mode='HTML')
    except Exception as e:
        print(f"⚠️ Ошибка регистрации пользователя: {e}")
    
    # Отправляем главное меню (БЕЗ упоминания токенов)
    send_main_menu(message.chat.id)
    print(f"✅ Главное меню отправлено для {user_id} (новый={is_new_user})")
