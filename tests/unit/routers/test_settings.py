...
class TestCbNotifications:
    @pytest.mark.asyncio
    async def test_shows_notification_toggles(self, mock_callback: MagicMock, user: User) -> None:
...
class TestCbToggleNotify:
    @pytest.mark.asyncio
    async def test_toggles_publications(self, mock_callback: MagicMock, user: User, mock_db: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_toggles_balance(self, mock_callback: MagicMock, user: User, mock_db: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_unknown_type_shows_alert(self, mock_callback: MagicMock, user: User, mock_db: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_update_failure_shows_alert(self, mock_callback: MagicMock, user: User, mock_db: MagicMock) -> None:
        mock_callback.data = "settings:notify:publications"
        with patch("routers.settings.UsersRepository") as repo_cls:
    ...
    @pytest.mark.asyncio
    async def test_answer_contains_status_text(self, mock_callback: MagicMock, user: User, mock_db: MagicMock) -> None:
