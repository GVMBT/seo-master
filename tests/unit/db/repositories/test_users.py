...
class TestGetById:
    ...
    async def test_returns_none_when_not_found(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
...
class TestGetOrCreate:
    ...
    async def test_creates_new_user(self, repo: UsersRepository, mock_db: MockSupabaseClient, user_row: dict) -> None:
...
class TestUpdate:
    async def test_partial_update(self, repo: UsersRepository, mock_db: MockSupabaseClient, user_row: dict) -> None:
...
class TestChargeBalance:
    async def test_rpc_success(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
        mock_db.set_rpc_response("charge_balance", [1000])
        result = await repo.charge_balance(123456789, 500)
        assert result == 1000

    async def test_rpc_success_dict_format(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
        # PostgREST may return [{"charge_balance": 1000}]
        mock_db.set_rpc_response("charge_balance", [{"charge_balance": 1000}])
        result = await repo.charge_balance(123456789, 500)
        assert result == 1000

    async def test_rpc_insufficient_balance_raises(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
    ...
    async def test_fallback_missing_user_raises(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
        mock_db.set_response("users", MockResponse(data=None))
...
class TestRefundBalance:
    ...
    async def test_fallback_success(self, repo: UsersRepository, mock_db: MockSupabaseClient, user_row: dict) -> None:
...
class TestForceDebitBalance:
    async def test_uses_charge_if_balance_sufficient(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
...
class TestGetReferralCount:
    async def test_returns_count(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
        mock_db.set_response("users", MockResponse(data=[], count=5))
        count = await repo.get_referral_count(123456789)
        assert count == 5

    async def test_returns_zero_when_none(self, repo: UsersRepository, mock_db: MockSupabaseClient) -> None:
