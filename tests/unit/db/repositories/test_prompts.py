...
class TestGetActive:
    async def test_found(
        self, repo: PromptsRepository, mock_db: MockSupabaseClient, prompt_row: dict
    ) -> None:
    ...
    async def test_not_found(
        self, repo: PromptsRepository, mock_db: MockSupabaseClient
    ) -> None:
...
class TestUpsert:
    async def test_upsert(
        ...
        data = PromptVersionCreate(
            task_type="article", version="v1", prompt_yaml="test yaml", is_active=True
        )
