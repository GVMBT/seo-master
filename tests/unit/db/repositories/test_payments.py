...
class TestCreatePayment:
    async def test_create(self, repo: PaymentsRepository, mock_db: MockSupabaseClient, payment_row: dict) -> None:
        ...
        data = PaymentCreate(user_id=123456789, provider="stars", tokens_amount=500, package_name="start")
        payment = await repo.create(data)
        assert isinstance(payment, Payment)
        assert payment.tokens_amount == 500
...
class TestGetByUser:
    async def test_returns_list(self, repo: PaymentsRepository, mock_db: MockSupabaseClient, payment_row: dict) -> None:
        mock_db.set_response("payments", MockResponse(data=[payment_row]))
...
class TestGetActiveSubscription:
    async def test_found(self, repo: PaymentsRepository, mock_db: MockSupabaseClient, payment_row: dict) -> None:
    ...
    async def test_not_found(self, repo: PaymentsRepository, mock_db: MockSupabaseClient) -> None:
...
class TestCreateExpense:
    async def test_create(self, repo: PaymentsRepository, mock_db: MockSupabaseClient, expense_row: dict) -> None:
        mock_db.set_response("token_expenses", MockResponse(data=[expense_row]))
        data = TokenExpenseCreate(user_id=123456789, amount=-320, operation_type="article")
...
class TestGetExpensesByUser:
    async def test_returns_list(self, repo: PaymentsRepository, mock_db: MockSupabaseClient, expense_row: dict) -> None:
        mock_db.set_response("token_expenses", MockResponse(data=[expense_row]))
