...
async def test_step02_navigate_to_projects(telethon_client, bot_username: str, clean_state) -> None:
    ...
    assert any(w in text for w in ["проект", "создать", "нет проектов", "список"]), (
...
async def test_step03_click_create_project(telethon_client, bot_username: str, clean_state) -> None:
    ...
    text = (result.text or "").lower()
    assert any(w in text for w in ["шаг 1", "название", "введите"]), f"Expected FSM step 1 prompt: {result.text!r}"
...
async def test_step04_enter_project_name(telethon_client, bot_username: str, clean_state) -> None:
    ...
    response = await send_and_wait(telethon_client, bot_username, _TEST_PROJECT_NAME, timeout=15.0)
    assert response is not None, "Bot did not respond to project name input"

    text = (response.text or "").lower()
    assert any(w in text for w in ["шаг 2", "компани", "название компании"]), f"Expected FSM step 2: {response.text!r}"
...
async def test_step05_enter_company_name(telethon_client, bot_username: str, clean_state) -> None:
    ...
    response = await send_and_wait(telethon_client, bot_username, _TEST_COMPANY, timeout=15.0)
    assert response is not None, "Bot did not respond to company name"

    text = (response.text or "").lower()
    assert any(w in text for w in ["шаг 3", "специализац", "опишите"]), f"Expected FSM step 3: {response.text!r}"
...
async def test_step06_enter_specialization(telethon_client, bot_username: str, clean_state) -> None:
    ...
    response = await send_and_wait(telethon_client, bot_username, _TEST_SPEC, timeout=15.0)
    assert response is not None, "Bot did not respond to specialization"

    text = (response.text or "").lower()
    assert any(w in text for w in ["шаг 4", "url", "сайт", "пропустить"]), f"Expected FSM step 4: {response.text!r}"
...
async def test_step08_project_card_has_data(telethon_client, bot_username: str, clean_state) -> None:
    ...
    assert any(w in all_text for w in [_TEST_PROJECT_NAME.lower(), _TEST_COMPANY.lower(), "карточка", "проект"]), (
    ...
    has_inline = any(m.reply_markup and hasattr(m.reply_markup, "rows") for m in all_msgs)
...
async def test_step09_delete_project_cleanup(telethon_client, bot_username: str, clean_state) -> None:
    ...
    if any(w in text for w in ["подтвер", "уверен", "удалить"]):
        ...
        if confirm_result is not None:
            ...
            assert any(w in confirm_text for w in ["удалён", "удален", "удалено", "проект"]), (
                f"Expected deletion confirmation: {confirm_result.text!r}"
            )
    else:
        # Direct deletion without confirmation
        assert any(w in text for w in ["удалён", "удален", "удалено"]), (
